...
public class DigitalIdentityCertificatePanel extends JPanel implements ContentDialogCloser {
	...
	private void loadCertificate(File file) {
		try {
			...
			CertificateToken cert = DSSUtils.loadCertificate(inputStream);
	...
	public void refresh() {
		...
		if (digitalIdentityModel != null) {
			CertificateToken certificate = digitalIdentityModel.getCertificate();
			if (certificate != null) {
				...
				List<String> keyUsages = decodeKeyUsages(certificate.getCertificate().getKeyUsage());

				try {
					List<String> extendedKeyUsages = certificate.getCertificate().getExtendedKeyUsage();
