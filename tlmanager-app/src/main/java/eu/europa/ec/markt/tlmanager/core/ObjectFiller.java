...
public class ObjectFiller {
	...
	public static TSPServiceInformationType fillTSPServiceInformationType(TSPServiceInformationType serviceInfo) throws FillerException {
		try {
			...
			final String[] tslServiceIdentifier = Configuration.getInstance().getTL().getTslServiceIdentifier();
			if (!Arrays.asList(tslServiceIdentifier).contains(serviceTypeIdentifier)) {
