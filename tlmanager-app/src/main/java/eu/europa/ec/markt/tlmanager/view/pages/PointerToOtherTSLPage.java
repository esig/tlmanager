...
import eu.europa.ec.markt.tlmanager.view.binding.ServiceDigitalIdentitiesConverter;
...
public class PointerToOtherTSLPage extends TreeDataPublisher {
	...
	private static final Logger LOG = LoggerFactory.getLogger(PointerToOtherTSLPage.class);
	...
	public void updateViewFromData(TSLDataNode dataNode) {
		...
		OtherTSLPointerType pointer = (OtherTSLPointerType) dataNode.getUserObject();
		LOG.trace("Value changed {}", pointer);
		...
		}
		{
			...
			if (Configuration.getInstance().isTlMode() && concreteElement != null && concreteElement.getValue() != null && concreteElement.getValue()
				  .getURI() != null && concreteElement.getValue().getURI().isEmpty()) {
